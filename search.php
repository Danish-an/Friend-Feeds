	...
	<?php  
		...
		else
		{
			...
			else
			{
				$names=explode(" ", $query);
				if (count($names) == 3)
					$usersReturnedQuery=mysqli_query($con,"SELECT * FROM users WHERE (first_name LIKE '$names[0]%' AND last_name LIKE '$names[2]%') AND user_closed='no'");  //name[0] =first character
				...
				if($user['username'] != $row['username'])
				{
					...
					if(isset($_POST[$row['username']]))
					{
						if($user_obj->isFriend($row['username']))
						{
							...
							header("Location: http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"); //current page ko refresh after sending request
						}
						else if($user_obj->didReceiveRequest($row['username']))
						{
