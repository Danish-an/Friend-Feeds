<?php 
	...
	{	
		...
		$uploadOk=1; //flag thpe variavle upload=1 or 0
		$imageName=$_FILES['fileToUpload']['name'];
		$errorMessage="";

		if($imageName != "") //img store local
		{
			$targetDir = "assets/images/posts/";
			$imageName = $targetDir . uniqid() . basename($imageName);//uniquid iniqui id 
	...
	<script>
		...
		$(document).ready(function ()
		{
			...
			$.ajax({
				...
				type: "POST",
				data: "page=1&userLoggedIn=" + userLoggedIn, //ek page mein sub display
				cache: false, //load faster

				success: function(data)
				{
					$('#loading').hide();
					$('.posts_area').html(data); // post area display in html data
			...
			$(window).scroll(function()
			{
				...
				if((document.body.scrollHeight == document.body.scrollTop + window.innerHeight) && noMorePosts == 'false')
				{
					...
					url: "includes/handlers/ajax_load_posts.php",
					type: "POST",
					data: "page=" + page + "&userLoggedIn=" + userLoggedIn, //value of pge is increafed height is increased page is only 1
					cache: false,
